...
function twoPassCodeTemplate(): string {
	...
	return [
		...
		"```",
		"",
		"This whole script is the `workflowScript` argument of exactly ONE top-level `subagent` tool call. That",
		"call MUST set `async: true` (plus a short `name`). Do not omit `async` and do not pass `async: false` --",
		"either one makes this a blocking call that stalls the conversation, sometimes for many minutes, with",
		"nothing visible to the user until it's done. With `async: true` the tool call returns immediately with a",
		"run receipt; you'll be resumed automatically via notification when pass 2 finishes, no need to poll or",
		"call subagent_wait for this.",
		"",
		"On parameters: the top-level `subagent` call takes `workflowScript`, `name`, and `async: true`,",
		"nothing else. `gate` and `acceptance` are mutually-exclusive evidence gates for write runs; this",
		"review is read-only, so neither applies. If the validator rejects the launch, don't keep retrying in",
		"this conversation -- rejected calls get copied back out of the history. Say so, and redo the",
		"launch in a fresh conversation where the history is clean.",
	].join("\n");
}
...
function twoPassInstructions(diffInstruction: string): string[] {
	...
	return [
		...
		"",
		"**Before launching anything**, send the user one short plain-text message saying the N-model review is starting in the background and you'll report back -- this is a real background job that can take a few minutes, not something to sit silently waiting on.",
		"",
